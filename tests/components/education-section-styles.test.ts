...
import educationSectionSource from '../../src/components/EducationSection.astro?raw';
...
const styles = (educationSectionSource.match(/<style[^>]*>([\s\S]*?)<\/style>/)?.[1] ?? '').replace(/\/\*[\s\S]*?\*\//g, '');
...
describe('EducationSection styles', () => {
  it('has scoped styles', () => {
    expect(educationSectionSource).not.toMatch(/<style[^>]*is:global/);
  ...
  it('stacks the cards below 768px and uses two equal columns from 768px', () => {
    expect(bodyOf(/^\.education-list$/, base)).toMatch(/gap:\s*var\(--space-3\)/);
    const desktop = bodyOf(/^\.education-list$/, (rule) => inside(rule, DESKTOP));
    expect(desktop).toMatch(/grid-template-columns:\s*repeat\(\s*2,\s*minmax\(0,\s*1fr\)\s*\)/);
    expect(desktop).toMatch(/gap:\s*var\(--space-4\)/);
  });

  it('draws each entry as a system card', () => {
    const card = bodyOf(/^\.education-card$/, base);
  ...
  it('shows the years with the mono label tokens and the specialization as secondary text', () => {
    const years = bodyOf(/^\.education-years$/, base);

    for (const token of ['--font-mono', '--text-mono-label-size', '--text-mono-label-weight', '--text-mono-label-tracking']) {
      expect(years).toContain(`var(${token})`);
    }
    expect(years).toMatch(/color:\s*var\(--color-text-muted\)/);
    expect(bodyOf(/^\.education-specialization$/, base)).toMatch(/color:\s*var\(--color-text-secondary\)/);
  });

  it('puts the institution at the bottom, with the abbreviation as a system tag', () => {
    expect(bodyOf(/^\.education-institution$/, base)).toMatch(/margin(-top)?:\s*auto/);
    expect(bodyOf(/^\.education-institution$/, base)).toMatch(/color:\s*var\(--color-text-muted\)/);

    const tag = bodyOf(/^\.education-short$/, base);
  ...
  describe('spotlight (M-1)', () => {
    ...
    it('brightens the years and the institution for the active card, and on hover with reduced motion', () => {
      ...
      for (const group of [active, hover]) {
        const selectors = group.map(({ selector }) => selector).join(',');
        expect(selectors).toMatch(/\.education-years/);
        expect(selectors).toMatch(/\.education-institution/);
        for (const { body } of group) expect(body).toMatch(/color:\s*var\(--color-text-secondary\)/);
      }
    });
